...
extension UIButton {
    ...
    func setImageWithStarndardColor(_ image: UIImage) {
        self.setImage(image.tint(color: Color.buttonNormal.UIColor), for: .normal)
        self.setImage(image.tint(color: Color.buttonNormal.UIColor, alpha: 0.4), for: .highlighted)
    }
}
