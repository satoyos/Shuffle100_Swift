...
extension VolumeSetting: View {
      ...
      VStack {
        HStack(alignment: .bottom, spacing: 0) {
          ZStack(alignment: .trailing) {
            Text("100")
              .monospacedDigit()
              .font(.system(size: digitSize(for: geometry.size.width)+8, weight: .medium))
              .opacity(0)
            Text(viewModel.output.ratioText)
              .monospacedDigit()
              .font(.system(size: digitSize(for: geometry.size.width), weight: .medium))
          }
          Text("%")
            .font(.system(size: digitSize(for: geometry.size.width) / 4, weight: .medium))
            .padding(.trailing, digitSize(for: geometry.size.width) / 2)
        }
        .padding()
        ...
        .padding(.top, digitSize(for: geometry.size.width) * 0.5)
  ...
  private func digitSize(for width: Double) -> Double {
    width / 5.0
  }
}
